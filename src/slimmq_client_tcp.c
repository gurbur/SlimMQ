...
#define FRAME_HEADER_SIZE 4  // 4바이트 프레임 길이 헤더
...
static int recv_framed_packet(int sockfd, uint8_t* buffer, size_t bufsize) {
    ...
    if (received != packet_len) return -1;

    return received;
}

// framing 방식으로 전송
static int send_framed_packet(int sockfd, const uint8_t* data, size_t data_len) {
    uint32_t packet_len = htonl(data_len);
    if (send(sockfd, &packet_len, FRAME_HEADER_SIZE, 0) != FRAME_HEADER_SIZE) return -1;
    if (send(sockfd, data, data_len, 0) != (int)data_len) return -1;
    return 0;
}
...
static void* listener_loop(void* arg) {
    ...
    while (client->running) {
        int len = recv_framed_packet(client->sockfd, buffer, sizeof(buffer));
        if (len <= 0) continue;

        if (deserialize_message(buffer, len, &header, topic_buf, sizeof(topic_buf),
        ...
        switch (header.msg_type) {
            ...
            case MSG_CONTROL: {
                ...
                char ctrl_data[256];
                if (deserialize_control_message(buffer, len, &header, &ctrl_type,
                                                ctrl_data, sizeof(ctrl_data)) == 0) {
            ...
            case MSG_PUBLISH:
                event_queue_push(&client->event_queue, MSG_PUBLISH, header.msg_id, topic_buf,
                                 payload_buf, header.payload_length - (1 + strlen(topic_buf)));
                break;
...
slimmq_client_t* slimmq_connect(const char* broker_ip, uint16_t port) {
    ...
    client->sockfd = init_socket(broker_ip, port, false);
    if (client->sockfd < 0) {
        free(client);
        return NULL;
    }
    ...
    inet_pton(AF_INET, broker_ip, &client->broker_addr.sin_addr);
    ...
    pthread_create(&client->listener_thread, NULL, listener_loop, client);
...
int slimmq_subscribe(slimmq_client_t* client, const char* topic) {
    slim_msg_header_t header = {
        .version = 1, .msg_type = MSG_SUBSCRIBE,
        .qos_level = QOS_AT_MOST_ONCE, .msg_id = client->next_msg_id++,
        .payload_length = 1 + strlen(topic),
        .topic_id = 0, .frag_id = 0, .frag_total = 1,
        .batch_size = 1, .client_node_count = 1
    };
    ...
    return send_framed_packet(client->sockfd, buffer, len);
}
...
int slimmq_publish(slimmq_client_t* client, const char* topic, const void* data, size_t data_len) {
    ...
    slim_msg_header_t header = {
        .version = 1, .msg_type = MSG_PUBLISH,
        .qos_level = client->qos_level, .msg_id = client->next_msg_id++,
    ...
    while (retries <= client->max_retries) {
        ...
        if (header.qos_level == QOS_EXACTLY_ONCE) {
            ...
            if (qos2_table_get_state(header.msg_id) != QOS2_CLIENT_STATE_WAIT_COMPLETE) {
                retries++;
                continue;
            }
            ...
            int ctrl_len = serialize_control_message(&rel_hdr, CONTROL_RELEASE, NULL, 0,
...
int slimmq_receive(slimmq_client_t* client, char* out_topic,
    ...
    int received = recv_framed_packet(client->sockfd, buffer, sizeof(buffer));
    if (received < 0) return -1;

    return deserialize_message(buffer, received, &header,
                               out_topic, topic_buf_size,
                               out_data, data_buf_size);
}
